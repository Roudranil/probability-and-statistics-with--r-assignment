...
NegLogLikeC = function(theta,data){
  ...
  for(i in 1:n){
    ...
    l = l + log(dlnorm(data$Claims[i],meanlog = mu, sdlog = log(sigma)))
    print(l)
  }
  return(-l)
}
...
theta_initial=c(0.001,0.1,log(10))
...
NegLogLikeD = function(theta,data){
  ...
  l=0
  for(i in 1:n){
    mu = exp(theta[1]+theta[2]*log(data$Holders[i]))
...
theta_initial=c(8,1,10)
NegLogLikeD(theta_initial,Insurance)

fit = optim(NegLogLikeD,
            theta_initial,
            data=Insurance)
ggplot(data=Insurance)+
  geom_line(aes(Holders, fit$par[1]+fit$par[2]*Holders))+

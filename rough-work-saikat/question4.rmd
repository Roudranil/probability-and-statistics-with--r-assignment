...
NegLogLikeA = function(theta,data){
  ...
  for(i in 1:n){
    ...
    l = l + dnorm(data$Claims[i],mean = mu,sd=sigma)
  }
  return(-l)
}
...
NegLogLikeB = function(theta, data){
  ...
  for(i in 1:n){
    mu = theta[1]+theta[2]*data$Holders[i]
    l = l + dlaplace(data$Claims[i], 0, sigma)
    print(l)
  }
  return(-l)
}
...
NegLogLikeC = function(theta,data){
  ...
  l=0
  for(i in 1:n){
    mu = theta[1]+theta[2]*log(data$Holders[i])
    l = l + dlnorm(data$Claims[i],meanlog = mu, sdlog = sigma)
    print(l)
  }
  return(-l)
}
...
```{r results = FALSE, warning=FALSE}
BIC=2*NegLogLikeC(theta_hat,Insurance)+log(nrow(Insurance))*2
```
...
NegLogLikeD = function(theta,data){
  ...
  for(i in 1:n){
    ...
    l = l + dgamma(data$Claims[i], shape = shape, scale = scale)
    print(l)
  }
  return(-l)
}
...
Comparing BIC of all the models, we see that BIC of model 1 is least.    
So, we conclude that model 1 is the best fit.
